...
from hifast.io import rec_his,save_dict_hdf5
import os
import sys

def down(properties,ra,dec,mjd,freq,vel,chanfactor=None,specfactor=None):
...
fileout= os.path.join(outdir, '.'.join(os.path.basename(fname).split('.')[:-1]) +'-downsample.hdf5' )
...
print(f"Origin[key].shape={properties[key].shape}")
...
#down sample
properties,ra,dec,mjd,freq,vel = down(properties,ra,dec,mjd,freq,vel,chanfactor,specfactor)
