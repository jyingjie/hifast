...
__all__ = ['replace_spec', 'repalce_near', 'replace_margin_side', 'get_margin_num', 'replace_rfi_lower', 'replace_rfi',
           'find_loc', 'SW_FFT', 'get_sw_conf', 'fit_sw_fft', 'mean_fit_ripple', 'med_fit_ripple', 'fit_ripple']
...
class SW_FFT(object):
    ...
    def gen_amp_thr_s(self, amp_thr_mean_factor=1.1, amp_thr_solo_factor=1.4,
        ...
        if is_on is None:
            ...
            self.amp_thr_solo[~is_on] = np.nanmedian(np.nanmean(self.amp[~is_on], axis=0)) * amp_thr_solo_factor
            self.amp_thr_solo[is_on] = np.nanmedian(np.nanmean(self.amp[is_on], axis=0)) * amp_thr_solo_factor
    ...
    def choose_and_ifft(self, method='interpolate', inplace_amp=True, sw_base=True):
        ...
        if method == 'interpolate':
            ...
            # no need to process apm[is_use_sw_chan_amp]
            pass
        if sw_base:
...
def comp2chan(freq,comp):
    dx = 1/(freq[1]-freq[0])/len(freq)
    comp /= 2
    if comp > 0: 
        chan = int(np.around(comp / dx))
    else:
        chan = 0
    chan_num = 2*chan-1 if chan > 0 else 0
...
def fit_sw_fft(s1p, freq, nproc, is_on=None, amp_thr_mean_factor=1.05, amp_thr_solo_factor=1.4, is_excluded_mean=None,
               comp_wide=0.09, comp_narr=0.03, sw_periods=['1mhz', '2mhz', '0_04mhz'],
