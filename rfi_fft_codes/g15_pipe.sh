...
python cli_markRFI.py $fname --outdir  $outdir \
        ...
        --rfi_thr 12 --rms_frange 1390 1400 --mw_frange 1419 1425 --rfi_groups 'two groups' \
        --rfi_width_lim 20 --ext_sec 20 --freq_thr .3 --freq_step 8.1 \
        --mask_RFI_method 'fixed freq' --ext_edge 3 --mask_thr 5 --freq_from_theory .5 \
        --plot --ylim -1 5 --save_sf -f --keep_polar || exit 1 
done
...
do
echo $fname
python cli_baseline_fft.py $fname --outdir $outdir --frange 1320 1440  \
...
#: << EOF
########################
## fifth, flux and velocity correction
...
echo "5. finish flux and velocity correction"
#EOF
#: << EOF
#######################
## sixth, down sample
...
mkdir $outdir
for fname in drift_$l/cor_vel/*T-bld-fft_bldr-bld-flux_fc.hdf5
do
echo $fname
