...
```hifast.cli_radec```: 转换馈源（KY）位置到RA DEC
...
* 程序会先依次检测是否存在 *```your_HOME_dir/KY/```*, */data/inspur_disk06/fast_data/KY/*, */data31/KY/* 文件夹。 然   后在最先检测到的文件夹里自动猜测对应KY的文件。
* 其他参数：
  * ```--kyfiles```：如果程序不能成功找到对应KY文件，加此参数来手动指定KY文件。参数后加KY文件的绝对路径。
  * ```--outdir``` ：指定输出文件存放的目录。如果输入hdf5文件默认与输入文件一致，如果输入.xlsx文件则默认为程序运行路径。
  * ```--loose```: 早期的一些Drift观测，馈源舱文件只记录开始的几分钟内的馈源舱位置，加此参数只利用这开始的几分钟来计算整个观测的      RADEC。由于Drift过程中馈源舱并不能完全稳定不动，这样计算出的RA DE会损失精度。
