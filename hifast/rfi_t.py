...
def get_rfi_folder(freq, T2p, folder, times, ext_add=None):
    rms_times = times
...
def get_rfi_c(T2p, n_continue=50, s_sigma=10, chan_smooth_method='gaussian', times_s=2, times=5, ext_add=None):
    """
    ...
    if ext_add is not None:
        is_exceeded = extend_Trues(is_exceeded, axis=1, ext_add=ext_add)
    #return is_exceeded
