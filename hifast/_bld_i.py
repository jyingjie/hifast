# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/03__bld_i.ipynb (unless otherwise specified).
...
class Test(object):
    ...
    def select(self, start=0, length=20, polar=0):
        self.T2p_t = self.T2p[start:start+length][:, :, polar:polar+1]
...
def main():
    ...
    sliders.update(_BoundedIntText(
        start=(0, 0, tes.T2p.shape[0]-1-length, 1), polar=(0, 0, 1, 1)))
    ...
    if isinstance(ylim, str):
        if ylim == 'auto':
            ylim = phrase_ylim(['per0.1', 'per99.9'], tes.T2p_t)
        elif ylim == 'full':
            ylim = 'fixed'
    else:
    ...
    hbs = [
        ...
        widgets.HBox([widgets.Label(
            value=f"4. Show the stack spectra of in the range in th bottom panel:"), w['start_stop']]),
    ]
