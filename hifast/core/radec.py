# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/02_radec.ipynb (unless otherwise specified).

__all__ = ['plot_radec', 'guess_ky', 'process_ky', 'kydata2radec', 'radec_interp', 'get_radec']
...
def process_ky(ky_files, mjds_src=None, tol=0, ky_fixed=False):
    ...
    if not this:
        print(f"input mjd are not in the mjd range of those KY files")
    else:
        print(f"using {ky_file}")
        return ky_data, mjd, ky_file

# Cell
def kydata2radec(ky_data, mjd, nBs='All', ky_fixed=False, nproc=1):
    """
...
def get_radec(parse_mjd, guess_str=None, ky_files=None, tol=0.3, ky_fixed=False, use_cache=True, nproc=1):
    ...
    if radec_ky is None:
        ...
        if ky_fixed:
            return kydata2radec(ky_data, mjds_src, ky_fixed=ky_fixed, nproc=nproc)
        else:
            radec_ky= kydata2radec(ky_data, mjds_ky, ky_fixed=ky_fixed, nproc=nproc)
