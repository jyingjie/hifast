# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/core/corr_vel.ipynb (unless otherwise specified).

__all__ = ['get_vcorr', 'freq2vel', 'frame_correct_freq', 'vlight', 'spectrum_resample', 'correct_spec']
...
def freq2vel(freq, vtype='radio'):
    ...
    if vtype == 'radio':
        vel = vlight*(restfreq - freq)/ restfreq
    elif vtype == 'optical':
        vel = vlight*(restfreq - freq)/ freq
    return vel
...
def _freq_vcorr(freq, vcorr):
    ...
    """
    return freq * np.sqrt((vlight - vcorr)/(vlight + vcorr))
